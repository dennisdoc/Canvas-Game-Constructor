...
function initSheetClass(){
	return {img: null,url: "",sprites: [],init: function () {},load: function (imgName) {this.url = imgName;var img = new Image();img.src = imgName;this.img = img;gSpriteSheets[imgName] = this;},defSprite: function (name, x, y, w, h, cx, cy) {var spt = {"id": name,"x": x,"y": y,"w": w,"h": h,"cx": cx === null ? 0 : cx,"cy": cy === null ? 0 : cy};this.sprites.push(spt);},parseAtlasDefinition: function (atlasJSON) {var parsed = atlasJSON;for(var key in parsed.frames) {var sprite = parsed.frames[key];var cx = -sprite.frame.w * 0.5;var cy = -sprite.frame.h * 0.5;if(sprite.trimmed){cx= sprite.spriteSourceSize.x-(sprite.sourceSize.w*0.5);cy= sprite.spriteSourceSize.y-(sprite.sourceSize.h*0.5);}this.defSprite(sprite.filename, sprite.frame.x, sprite.frame.y, sprite.frame.w, sprite.frame.h, cx, cy);}},getStats: function (name) {for(var i = 0; i < this.sprites.length; i++) {if(this.sprites[i].id === name) { return this.sprites[i];}}return null;}};
}
...
function drawSprite(spritename, posX, posY,tileSize) {
	...
	for(var sheetName in gSpriteSheets) {
		...
		__drawSpriteInternal(sprite, sheet, posX, posY,tileSize);
...
// canvas to draw to.
function __drawSpriteInternal(spt, sheet, posX, posY,tileSize) {
	...
	ctx.drawImage(sheet.img, spt.x, spt.y, spt.w, spt.h, posX, posY,tileSize, tileSize);
